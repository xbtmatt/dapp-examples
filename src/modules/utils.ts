...
export async function getTxByHash(txHash: string, customNodeUrl?: string) {
    const response = await fetch(
        `${
            customNodeUrl ?? process.env.NEXT_PUBLIC_NODE_URL
        ...
        try {
            ...
            return Promise.resolve(
                Number(resource.data["coin"]["value"]) /
                    (process.env.NEXT_PUBLIC_APTOS_COIN_FACTOR
                        ? parseInt(process.env.NEXT_PUBLIC_APTOS_COIN_FACTOR)
                        : 100000000),
            );
        } catch (e) {
            console.error(e);
...
export function getCountdown(ms: number, countup?: boolean): Countdown {
    ...
    const seconds = Math.floor(
        (timeDifference - (hours * MS_IN_AN_HOUR + minutes * 60 * 1000)) / 1000,
    );
...
export async function awaitTransaction(
    ...
    while (true) {
        ...
        if (
            response.type === "pending_transaction" ||
            response.error_code === "transaction_not_found"
        ) {
            count += 1;
            if (count >= (tries ?? 10)) {
