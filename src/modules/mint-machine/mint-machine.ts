...
export const RESOURCE_ACCOUNT_ADDRESS = process.env.MINT_MACHINE_MODULE_ADDRESS!;
...
export const initializeMintMachinePayload = (
...
export const addTokensPayload = (props: AddTokensProps): TxnBuilderTypes.TransactionPayloadEntryFunction => {
  ...
  props.propertyTypes = Array.isArray(props.propertyTypes)
    ? props.propertyTypes
    : (createTypedArray(props.propertyValues, props.propertyTypes) as Array<any>);
  // Ensure that the lengths of propertyValues and propertyTypes are the same.
  if (
    props.propertyKeys.length !== props.propertyValues.length ||
    props.propertyKeys.length !== props.propertyTypes.length
  ) {
...
export const enableMintingPayload = (): TxnBuilderTypes.TransactionPayloadEntryFunction => {
  return new TxnBuilderTypes.TransactionPayloadEntryFunction(
    TxnBuilderTypes.EntryFunction.natural(`${RESOURCE_ACCOUNT_ADDRESS}::mint_machine`, "enable_minting", [], []),
  );
};

export const enableMinting = async (provider: Provider, admin: AptosAccount): Promise<Types.UserTransaction> => {
  return await submitPayloadHelper({
    provider,
    account: admin,
...
export const viewTokensAddedByAdmin = async (provider: Provider, adminAddress: HexString): Promise<number> => {
  try {
...
export const viewCreatorObject = async (provider: Provider, adminAddress: HexString): Promise<HexString> => {
...
export const constructTypeTag = (accountAddress: HexString, moduleName: string, functionNameOrType: string): string => {
...
export const viewMintConfiguration = async (
  ...
  const mintConfigurationResource = (
...
export const viewAllowlistTierInfo = async (
  ...
  return {
    ...
    price: Number(res[1]),
    startTimestamp: new Date(Number(res[2]) * 1000),
    endTimestamp: new Date(Number(res[3]) * 1000),
...
export const viewReadyForLaunch = async (provider: Provider, adminAddress: HexString): Promise<ReadyForLaunch> => {
  const res = (
    await provider.view({
...
export const mintAndViewTokens = async (
  ...
  printTxResponse(
    await mintMultiple(provider, minter, {
      adminAddress,
      amount,
    }),
  );

  printView(await provider.indexerClient!.getOwnedTokens(minter.address()));
};

export const viewTokenUris = async (provider: Provider, adminAddress: HexString): Promise<Array<string>> => {
  const res = (
    await provider.view({
